...
### SeedreamClient
Seedream SDK 适配层，将 ArkService 生命周期、SDK 请求构建、响应解析隔离在接口之后。仅通过 `ImageGenerationRequest` / `ImageGenerationResult` 自有 DTO 与外部交互，不泄露 SDK 类型。错误内部消化，通过 `status==FAILED` 通知调用方。使 `SeedreamImageServiceImpl` 可通过 mock 独立测试。
...
### Remaining
额度中的剩余可用次数。免费赠送和付费充值合并为一个 remaining 字段，由 QuotaService 统一管理。扣减前检查 remaining > 0。

### QuotaService
用户额度管理接口，统一扣减（checkAndDeduct）、增加（addCredits）和初始化创建（create）操作。QuotaManagerImpl 是其唯一实现，独占 UserQuotaRepository，外部模块不再直接操作额度表。所有修改操作使用悲观锁（SELECT FOR UPDATE）保证并发安全。
...
### MockSmsService
SMS 短信验证码的占位实现，位于 `service.impl.mock` 包。实现 SmsService 接口，调用方通过接口注入，替换真实短信渠道时只需新增 adapter，AuthServiceImpl 零改动。

### MockPaymentServiceImpl
支付 SDK 的占位实现，位于 `service.impl.mock` 包。实现 PaymentService 接口，直接返回支付成功并生成模拟第三方交易流水号。后续替换真实支付 SDK 时只需新增 adapter。
